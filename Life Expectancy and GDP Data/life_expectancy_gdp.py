...
p1=sns.barplot(data=df,x='Country',y='GDP')
...
p2=sns.barplot(data=df,x='Country',y='LEABY')
...
ax.set_xticklabels(p1.get_xticklabels(),rotation=90)
...
#What years are there the biggest changes in the data?
In 2005, the life expectancy in Zimbabwe increased for the first time in the data.

#Which country has had the least change in GDP over time?
Zimbabwe

#How do countries compare to one another?
Compare the GPD growth and life expectancy of countries

#Now that you can see the both bar charts, what do you think about the relationship between GDP and life expectancy?
Global life expectancy at birth is projected to increase. 
On the contrary, data reveal the gap between zimbabwe and other countries, 

#Can you think of any reasons that the data looks like this for particular countries?
Yes, despite the rapid increase of population growth in Zimbabwe, GDP grouth is significantly slow.
...
#Is this surprising?
No, but multiple scatter plots reveal the results that hard to find in the bar plot.

#Do you think these scatter plots are easy to read? Maybe there's a way to plot that!
Difficult to said scatter plot is the best visualization for sconsecutive data
...
#What are your first impressions looking at the visualized data?
easy to compare the change of the life expectancy by year

#Which countries' line changes the most?
Zimbabwe

#What years are there the biggest changes in the data?
since 2005, Zimbabwe experience rapid population grouwth
...
g4 = (g4.map(plt.plot,"Year", "GDP",color="g").add_legend())
...
highest GDP: United states of America
lowest GDP: Zimbabwe
...
sns.barplot(data=df_china, x="Year",y="GDP") 
plt.title("Bar Plots Of GDP over time")

plt.savefig("lp_LEABY.png")
...
#####blog post#######
https://hirune33.blogspot.com/2018/07/the-economic-output-of-country-and-life.html
    
#How do you think the histories and the cultural values of each country relate to its GDP and life expectancy?
To compare histories and the cultural values from around the country, we need more measurable data.
It can be said that Zimbabwe tend to have a lower GDP and life expectancy than other countries.

#What would have helped make the project data more reliable? What were the limitations of the dataset?
It is expected to collect larger data to support a clear rationale.

#Which graphs better illustrate different relationships??
violin plot and line plot.

#1. The violin plot of the life expectancy distribution by country
fig = plt.subplots(figsize=(15, 10)) 
sns.violinplot(data=df,x='Country',y='LEABY')
plt.title("The violin plot of the life expectancy distribution by country")
#plt.savefig("vp_LEABY.png")
plt.show()

#2. The facet grid of scatter graphs mapping GDP as a function Life Expectancy by country
g = sns.FacetGrid(df, col="Year", hue="Country", col_wrap=4, size=2)
g = (g.map(plt.scatter,"GDP","LEABY", edgecolor="w").add_legend())
plt.suptitle("Scatter Plots of GDP and Life Expectancy",y=1.08)
#plt.savefig("sp_GDP_LEABY.png")

#3. The facet grid of line graphs mapping GDP by country
g2 = sns.FacetGrid(df, col="Country", col_wrap=3, size=4)
g2 = (g2.map(plt.plot,"Year", "GDP").add_legend())
plt.suptitle("Line Plots for GDP",y=1.08)
#plt.savefig("lp_GDP.png")

#4. The facet grid of line graphs mapping Life Expectancy by country
g3 = sns.FacetGrid(df, col="Country", col_wrap=3, size=4)
g3 = (g3.map(plt.plot,"Year", "LEABY").add_legend())
plt.suptitle("Line Plots for Life Expectancy",y=1.08)
#plt.savefig("lp_LEABY.png")
